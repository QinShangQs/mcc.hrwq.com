    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-header card-padding">
                <form action="{{ route('vcourse.marks') }}" method='GET'>
                    <div class="row">
                        <div class="col-sm-3">                       
                        ...
                        <div class="col-sm-3">
                            <div class="input-group form-group">
                                ...
                                <div class="dtp-container fg-line">
                                    <div class="select">
                                        <select class="selectpicker" name="visible">
                                            <option value="">全部</option>
                                            @foreach ($visible_list as $k=>$item)
                                                <option value="{{$k}}" @if(request('visible')==$k) selected @endif>{{$item}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                ...
                <table id="data-table-selection" class="table table-striped">
                    ...
                    @foreach ($data as $item)
                        <tr id="b{{$item->id}}">
                            ...
                            <td>{{ @$visible_list[$item->visible]}}</td>
                            <td>{{ $item->likes }}</td>
                            <td>
                                <a href="{{route('vcourse.mark_show',['id'=>$item->id])}}" title="详情"><button class="btn bgm-orange waves-effect"><i class="zmdi zmdi-eye"></i></button></a>
        ...
        function del_all() {
            ...
            if(ids == "")
            {
                ...
                swal({
                    ...
                    $.ajax({
                        type: 'post',
                        url: '{{route('vcourse.mark_delete')}}',
                        data: {id: ids},
                        dataType: 'json',
                        success: function (res) {
                            if(res.code == 0){
                                swal(res.message);
                                window.location.href="{{route('vcourse.marks')}}";
                            } else {
                                swal(res.message);
                            }
                        },
